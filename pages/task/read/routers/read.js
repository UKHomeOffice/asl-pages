...
module.exports = () => {
    ...
    if (model === 'project') {
      ...
      if (action === 'transfer') {
        ...
        return req
          ...
          .then(() => {
            if (
              req.project.transferEstablishmentId &&
              req.project.transferProjectId
            ) {
              const params = {
              ...
              return req.user.can('project.read.single', params).then((can) => {
                if (can) {
                  return req
                    .api(
                      `/establishment/${req.project.transferEstablishmentId}/project/${req.project.transferProjectId}`
                    )
                    .then(({ json: { data } }) => {
      ...
      return req
        .api(url, { query: { withDeleted: true } })
        .then(({ json: { data } }) => {
          req.project = data;
        })
        .then(() => {
  ...
  app.use((req, res, next) => {
    ...
    if (profileId && establishmentId) {
      return req
        .api(`/establishment/${establishmentId}/profile/${profileId}`)
        .then(({ json: { data } }) => {
  ...
  app.use((req, res, next) => {
    ...
    if (
      ...
      return req
        .api(getUrl(), { query: { withDeleted: true } })
        .then(({ json: { data } }) => {
  ...
  app.use((req, res, next) => {
    if (
      ...
      Promise.resolve()
        .then(() => {
          return req
            .api(`/establishment/${req.establishment.id}/named-people`)
            .then((result) => {
    ...
    form(
      Object.assign({
        configure: (req, res, next) => {
          res.locals.static.content = merge(
            {},
        ...
        locals: (req, res, next) => {
          ...
          res.locals.static.isInspector =
            req.user.profile.asruUser && req.user.profile.asruInspector;
          res.locals.static.endorsingOwnPil = endorsingOwnPil(
            req.task,
            req.user.profile
          );
  ...
  app.post('/', (req, res, next) => {
    ...
    console.log({model, action, status});
